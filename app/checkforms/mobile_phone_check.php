...
if(isset($_SESSION["idu"])){
		$id_set = 1;
		$idu = $_SESSION["idu"];
		$select_query1 = "SELECT * FROM `mobile_phone_check` WHERE `id` = $idu AND `clis_cms_id`='$cli_id'";
...
if($res){
      	 ...
      	 $sql = "UPDATE `mobile_phone_check` SET `date` = '$Date', `crew_name` = '$Crew_Name', `design` = '$Design', `train_no` = '$Train_no', `phone_no` = '$Phone_no', `remarks` = '$Remarks' WHERE `mobile_phone_check`.`id` = $idu AND `clis_cms_id`='$cli_id'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 2;
          }
      	...
      	$idu = $_POST["trans_id"];
      	$sql = "DELETE FROM `mobile_phone_check` WHERE `mobile_phone_check`.`id` = $idu AND `clis_cms_id`='$cli_id'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 3;
          }
...
<form name="mpc" method="post" action="">
	...
	//----------------------------LIST OF RECORDS----------------------------
		echo "<hr><br>";
		$select_query = "SELECT * FROM `mobile_phone_check` WHERE `date` LIKE '%$Date%' AND `crew_name` LIKE '%$Crew_Name%' AND `design` LIKE '%$Design%' AND `train_no` LIKE '%$Train_no%' AND `phone_no` LIKE '%$Phone_no%' AND `remarks` LIKE '%$Remarks%' AND `clis_cms_id`='$cli_id' order by `id` desc limit 5";
