...
if(isset($_SESSION["idu"])){
		$id_set = 1;
		$idu = $_SESSION["idu"];
		$select_query1 = "SELECT * FROM `safety_seminar` WHERE `id` = $idu AND `clis_cms_id`='$cli_id'";
...
if($res){
      	 ...
      	 $sql = "UPDATE `safety_seminar` SET `seminar_topic` = '$Seminar_topic', `date` = '$Date', `place` = '$Place', `lpm` = '$LPM', `lpp` = '$LPP', `lpg` = '$LPG', `lps` = '$LPS', `alp` = '$ALP' WHERE `safety_seminar`.`id` = $idu AND `clis_cms_id`='$cli_id'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 2;
          }
      	...
      	$idu = $_POST["trans_id"];
      	$sql = "DELETE FROM `safety_seminar` WHERE `safety_seminar`.`id` = $idu AND `clis_cms_id`='$cli_id'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 3;
          }
...
<form name="ss" method="post" action="">
	...
	//----------------------------LIST OF RECORDS----------------------------
		...
		//FRAMING QEUERY
		$select_query = "SELECT * FROM `safety_seminar` WHERE `seminar_topic` LIKE '%$Seminar_topic%' AND `date` LIKE '%$Date%' AND `place` LIKE '%$Place%' AND `clis_cms_id`='$cli_id'";
