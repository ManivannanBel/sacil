...
if(isset($_SESSION["idu"])){
		$id_set = 1;
		$idu = $_SESSION["idu"];
		$select_query1 = "SELECT * FROM `adj_division` WHERE `id` = $idu AND `clis_cms_id` like '$cli_id1'";
...
$res = mysqli_query($conn,$sql);
          if($res){
      	 ...
      	 $sql = "UPDATE `adj_division` SET `date` = '$Date', `loco_no` = '$Loco_no', `train_no` = 'Train_no' ,`from1` = '$From', `to1` = '$To', `lps_name` = '$LPs_Name', `remarks` = '$Remarks',`clis_cms_id`='$cli_id' WHERE `adj_division`.`id` = $idu AND `clis_cms_id` like '$cli_id1'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 2;
          }
      	...
      	$sql = "DELETE FROM `adj_division` WHERE `adj_division`.`id` = $idu AND `clis_cms_id` like '$cli_id1'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 3;
          }
...
<div class="panel-heading">
	<h4>Adjacent Division Monitoring</h4>
</div>
...
<form name="Adj" method="post" action="">
	...
	<input type="reset" name="Adjreset" class="btn btn-primary" value="Clear"/>
	<?php
		if($id_set==1){
	?>
	...
	//----------------------------LIST OF RECORDS----------------------------
		echo "<hr><br>";
		$select_query = "SELECT * FROM `adj_division` WHERE `date` LIKE '%$Date%' AND 
		`loco_no` LIKE '%$Loco_no%' AND
		 `from1` LIKE '%$From%' 
		 AND `to1` LIKE '%$To%' AND
		  `lps_name` LIKE '%$LPs_Name%' 
		  AND `remarks` LIKE '%$Remarks%'
		  AND `clis_cms_id` like '%$clicmsid%' 
		  AND `clis_cms_id`like'$cli_id1' 
