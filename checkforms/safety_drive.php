<?php
//------------------------------START OF SCRIPT -------
$Drive_topic=$Drive_Period1=$Drive_Period2=$LPM=$LPP=$LPG=$LPS=$ALP=$sddrivetype=$Drive_Desc=$topicID='';
//	TO UPDATE VALUE CHECK---------------------------------------
if(isset($_SESSION["idu"])){
		$id_set = 1;
		$idu = $_SESSION["idu"];
		$select_query1 = "SELECT * FROM `safety_drive` WHERE `id` = $idu AND `clis_cms_id` lIKE '$cli_id1'";
...
if($res){
 ...
 WHERE `safety_drive`.`id` = $idu AND `clis_cms_id` LIKE '$cli_id1'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 2;
          }
      ...
      //---------------------------DELETING DATA-----------------
      	...
      	$idu = $_POST["trans_id"];
      	$sql = "DELETE FROM `safety_drive` WHERE `safety_drive`.`id` = $idu AND `clis_cms_id` LIKE '$cli_id1'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 3;
          }
...
if(isset($_POST['getData'])) {
      	...
      	$fetch_topics_query = "SELECT * FROM topics WHERE `ID` = '$topicID'";
...
<form name="sd" method="post" action="">
	...
	<table name = "sdt1">
        ...
        <tr>
			<th>Drive topic: </th>
			<td><input type="text" class = "form-control" name="sdtopic" placeholder = "Drive Topic" <?php if(isset($topic_set) && $topic_set==1) echo "value='$topicTitle'"; ?>/></td>
			<td> 
				<button type="button" class="btn btn-link btn-sm" name="pic_topic" data-target="#popup" data-toggle="modal" style="padding-top:0px;">SELECT SAFETY-DRIVE TOPIC</button>
			  	<div class="modal" data-keyboard="false" data-backdrop="static" id="popup" tabindex="-1" style="margin-top:5px;">
              	<div class="modal-dialog modal-lg">
              	<div class="modal-content">
              	<div class="modal-header text-center">
                        <h4 class="modal-title text-primary">SAFETY DRIVE DETAILS</h4>
		                ...
		                <table class="table table-primary table-bordered  table-hover table-resoponsive"text-align="center">
		                     <thead class="text-warning">
		                            <tr class="table-primary">
		                                  <th width="130px auto;">SUBJECT</th>
		                                  <th width="100px auto;">TIME FROM</th>
		                                  <th width="100px auto;">TIME TO</th>
		                                  <th width="200px auto;">SUBJECT-DESCRIPTION</th>
		...
		<tr>
			<th>FROM</th>
			<td><input type="DATE" class = "form-control" name="sdperiod1" <?php if(isset($topic_set) && $topic_set==1) echo "value='$topicFrom'"; ?>/></td>
		</tr>
		<tr>
			<th>TO</th>
			<td><input type="DATE" class = "form-control" name="sdperiod2"  <?php if(isset($topic_set) && $topic_set==1) echo "value='$topicTo'"; ?>/></td>
		</tr>
		<tr>
			<th>DESCRIPTION: </th>
			<td><textarea rows="3" col="5"type="text" class="form-control" name="topic_description" style="resize:none;"><?php if(isset($topic_set) && $topic_set==1) echo $topicDesc; ?></textarea></td>
		</tr>
		</table>
