...
if(isset($_SESSION["idu"])){
		$id_set = 1;
		$idu = $_SESSION["idu"];
		$select_query1 = "SELECT * FROM `rr` WHERE `id` = $idu AND `clis_cms_id` like '$cli_id1'";
...
if(isset($_POST["RRsubmit"]) || isset($_POST["RRsubmitUpdate"])||isset($_POST["DeleteRecord"])||isset($_POST["modify"])){
	...
	if(isset($_POST["RRsubmit"])){
   	...
   	//--------------------------TO UPDATE RECORD---------------
      	 ...
      	 $sql = "UPDATE `rr` SET `date` = '$Date', `place` = '$Place', `remarks` = '$Remarks',`clis_cms_id`='$cli_id'  WHERE `rr`.`id` = $idu AND `clis_cms_id`like'$cli_id1';";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 2;
          }
    ...
    //---------------------------DELETING DATA-----------------
      	...
      	$idu = $_POST["trans_id"];
      	$sql = "DELETE FROM `rr` WHERE `rr`.`id` = $idu AND `clis_cms_id` like '$cli_id1'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 3;
          }
...
<form name="RR" method="post" action="">
	...
	<input type="reset" name="RRreset" class="btn btn-primary" value="Clear"/>
	<?php
		if($id_set==1){
	?>
...
//----------------------------LIST OF RECORDS----------------------------	
		echo "<hr><br>";
		//FRAMING QEUERY
		$select_query = "SELECT * FROM `rr` WHERE `date` LIKE '%$Date%' AND `place` LIKE '%$Place%' AND `remarks` LIKE '%$Remarks%' AND 
...
if(isset($_SESSION['loggeduserid_original'])){
				...
				echo '<a class="list-group-item" href="god.php?fname=rr&idu='.$id1.'">'."<b>id:</b> " . $row["id"]. "&nbsp<b>date:</b>" . format_output($row["date"]). "&nbsp<b>place:</b> " .format_output( $row["place"]). "&nbsp<b>remarks:</b> ".format_output($row["remarks"])."<span id ='controller_style'> $ctrl_name</span></a>";
			}
			echo '</div>';
		}else{
