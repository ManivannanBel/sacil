...
if(isset($_SESSION["idu"])){
		$id_set = 1;
		$idu = $_SESSION["idu"];
		$select_query1 = "SELECT * FROM `jss` WHERE `id` = $idu AND `clis_cms_id` like '$cli_id1'";
...
if($res){
      	 ...
      	 $sql = "UPDATE `jss` SET `date` = '$Date', `loco_no` = '$Loco_no',`train_no`='$Train_no',`timing`='$Timing' ,`section` = '$Section', `remarks` = '$Remarks' ,`clis_cms_id`='$cli_id' 
 WHERE `jss`.`id` = $idu AND `clis_cms_id` like '$cli_id1'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 2;
          }
      	...
      	$idu = $_POST["trans_id"];
      	$sql = "DELETE FROM `jss` WHERE `jss`.`id` = $idu AND `clis_cms_id` like '$cli_id1'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 3;
          }
...
<form name="JSS" method="post" action="">
	...
	<input type="reset" name="JSSreset" class="btn btn-primary" value="Clear"/>
	<?php
		if($id_set==1){
	?>
	...
	//----------------------------LIST OF RECORDS----------------------------
		echo "<hr><br>";
		$select_query = "SELECT * FROM `jss` WHERE `date` LIKE '%$Date%' AND `loco_no` LIKE '%$Loco_no%'  AND `train_no` LIKE '%$Train_no%' AND `section` LIKE '%$Section%' AND `remarks` LIKE '%$Remarks%' AND 
