...
if(isset($_SESSION["idu"])){
		$id_set = 1;
		$idu = $_SESSION["idu"];
		$select_query1 = "SELECT * FROM `sec/sfc` WHERE `id` = $idu AND `clis_cms_id`LIKE'$cli_id1'";
...
if($res){
      	 ...
      	 $sql = "UPDATE `sec/sfc` SET `lps_name` = '$LPs_Name', `conseled_date` = '$Conseled_Date', `remarks` = '$Remarks' ,`clis_cms_id`='$cli_id' WHERE `sec/sfc`.`id` = $idu AND `clis_cms_id` LIKE '$cli_id1'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 2;
          }
    ...
    //---------------------------END UPDATING------------------
      	...
      	$idu = $_POST["trans_id"];
      	$sql = "DELETE FROM `sec/sfc` WHERE `sec/sfc`.`id` = $idu AND `clis_cms_id` lIKE '$cli_id1'";
      	$res = mysqli_query($conn,$sql);
          if($res){
              $response_msg = 3;
          }
...
<form name="SEC" method="post" action="">
	...
	<input type="reset" name="SECreset" class="btn btn-primary" value="Clear"/>
	<?php
		if($id_set==1){
	?>
	...
	//----------------------------LIST OF RECORDS----------------------------
		echo "<hr><br>";
		$select_query = "SELECT * FROM `sec/sfc` WHERE `lps_name` LIKE '%$LPs_Name%' AND `conseled_date` LIKE '%$Conseled_Date%' AND `remarks` LIKE '%$Remarks%' AND 
...
if(isset($_SESSION['loggeduserid_original'])){
				...
				echo '<a class="list-group-item" href="god.php?fname=sec_sfc&idu='.$id1.'">'."<b>id:</b> " . $row["id"]. "&nbsp<b>lps_name:</b>" . format_output($row["lps_name"]). "&nbsp<b>conseled_date:</b> " . format_output($row["conseled_date"]). "&nbsp<b>remarks:</b> ".format_output($row["remarks"])."<span id ='controller_style'> $ctrl_name</span></a>";
			}
			echo '</div>';
		}else{
